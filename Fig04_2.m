...
Ptest_sigma_o = 0.1:0.1:0.6;
repeat = 100;
data_PEB = zeros(length(Ptest_sigma_o),1);
data_LB = zeros(repeat,length(Ptest_sigma_o));

%% ============================================= mismatch on RIS position
% calculate PEB
for i = 1:length(Ptest_sigma_o)
    % default setup
    sp = default_setup();
    % update setup
    sp.Pr_error = zeros(3,1);
    sp.Or_Euler_error = Ptest_sigma_o(i)*randn(3,1);
    sp = update_setup(sp); 
    % get PEB
...
for j = 1:repeat
    for i = 1:length(Ptest_sigma_o)
        waitbar((j-1)*length(Ptest_sigma_o)/(repeat*length(Ptest_sigma_o)),wb);
        % default setup
        sp = default_setup();
        % update setup
        sp.Pr_error = zeros(3,1);
        sp.Or_Euler_error = Ptest_sigma_o(i)*randerror(:,j);
        sp = update_setup(sp); 
        % get FIM
        FIM_ch = get_FIM_ch(sp);
        EFIM_ch = FIM_ch(1:4,1:4) - FIM_ch(1:4,5:8)*FIM_ch(5:8,5:8)^(-1)*FIM_ch(5:8,1:4);
...
figure(1)
semilogy(Ptest_sigma_o,data_PEB,'r--', 'LineWidth', 1); hold on
errorbar(Ptest_sigma_o, data_MCRB_mean, data_MCRB_mean-data_MCRB_min, data_MCRB_max-data_MCRB_mean, 'bo-', 'LineWidth', 1); hold off
legend('PEB','LB')
grid on;
xlabel('$\sigma_\mathrm{o}$ [deg]','interpreter','latex');
